...
void Currancy::addPurchase(string Modifier)
{
	...
	if (Modifier == "OLD")
	{
		...
		{
			system("cls");
			cout << "У вас нет более старых покупок, войдите без модификатора" << endl << endl;
			return;
		}
		if (Amount.size() >= 2)
		{
			...
			{
				system("cls");
				cout << "У вас есть несколько покупок с разными курсами, пожалуйста, ввдедите желаемый курс\n";
	...
	{
		cout << "Такого модификатора нет" << endl;
		return;
	}
}
